...
def Model(dims=96, batch_size=16, epochs=5):
    ...
    def getDataset():
        ...
        for i in range(p.getNumImage()):
            ...
            image_new = p.Grayscale(p.Crop(image_new,
                                           is_image=True,
                                           crop=(dims, dims)))
    ...
    def Preprocess_data():
        ...
        keypoints = keypoints.reshape(
                keypoints.shape[0], -1).astype(np.float32)
        keypoints_test = keypoints_test.reshape(
                keypoints_test.shape[0], -1).astype(np.float32)
        # Scaling the inputs
        inputs = inputs/255.
    ...
    def Train():
        ...
        model.compile(loss=root_mean_squared_error,
                      optimizer="adam",
                      metrics=["accuracy"])
        checkpoint = ModelCheckpoint(image_path+"new.model.best.hdf5",
                                     verbose=1,
                                     save_best_only=True)
        hist = model.fit(inputs, keypoints,
                         batch_size=batch_size, epochs=epochs,
                         callbacks=[checkpoint],
                         verbose=1,
                         shuffle=True,
                         validation_split=.2)
        score = model.evaluate(inputs_test,
                               keypoints_test,
                               verbose=0)
...
val_set, _ = DataLoader(validation_path,
                        resize_image=False,
                        dims=(dims, dims)).import_image()
